# code provided by Zixuan on Sep 16, 22  (file-name: zzx02.py)
# modified my Marek
...
if r_or_c == True:
    ...
    # input_data = pd.read_excel('../data/countries.xlsx')
    input_data = pd.read_excel('../data/china.xlsx')
    ...
    regions_or_countries = ['CHN']
    selected_regions_or_countries = [28]

if daily_time_slices_in_a_week == 2:
    from mm03 import algorithm as t48
    ...
    while region < total_regions:
        ...
        while s < yearly_divisions:
            ...
            SWHD_ms_lc_sorted[0,s,:] = np.sort(solar[int(s*8760/yearly_divisions):int((s+1)*8760/yearly_divisions)], axis=None)[::-1]   # 季节
            ...
            SWHD_h_ms_lc_sorted[0,s,:] = np.sort(SWHD_h_lc[0,int(s*8760/yearly_divisions/hours_in_a_time_slice):int((s+1)*8760/yearly_divisions/hours_in_a_time_slice+correction)], axis=None)[::-1]    # 周
    ...
    path_results = path+'China_Results/'+yearly_divisions_type+'_ts='+str(yearly_divisions)+' hourly_ts='+str(hourly_time_slices_in_a_day)+'/'
