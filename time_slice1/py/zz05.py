...
def algorithm(data, name, value, solarweight, windweight, hydroweight, demandweight, mid_night, path):
    ...
    while region < total_regions:
        ...
        solar = dt[:, 2 + 5 * value[region]]
        wind = dt[:, 1 + 5 * value[region]]
        #hydro = dt[:, 5 + 5 * value[region]]
    ...
    panda.to_excel('../Results/' + '48 Time Slices (Seasonal).xlsx')

    panda = pd.DataFrame(solar_and_wind_sorter_seasonal.T)  # 提取出的时间片段每个季节分别排序
    panda.to_excel('../Results/' + '48 Time Slices Sorted (Seasonal).xlsx')

    panda = pd.DataFrame(solar_and_wind_duration_curve_seasonal.T)  # 原始数据按季节排序后拼接
    panda.to_excel('../Results/' + 'Duration Curve (Seasonal).xlsx')

    panda = pd.DataFrame(solar_and_wind_annual.T)   # 提取出的时间片段按年排序
    panda.to_excel('../Results/' + '48 Time Slices (Annual).xlsx')

    panda = pd.DataFrame(solar_and_wind_sorter_annual.T)    # 提取出的时间片段按季节排序后又按年排序（结果与前一个相同）
    panda.to_excel('../Results/' + '48 Time Slices Sorted (Annual).xlsx')

    panda = pd.DataFrame(solar_and_wind_duration_curve_annual.T)    # 原始数据按年排序
    panda.to_excel('../Results/' + 'Duration Curve (Annual).xlsx')

    # Plotting seasonal results
    x_axis = np.arange(13)
    x2_axis = np.arange(2191)
    s_w_h_ds = ['Solar', 'Wind', 'Demand']
    ...
    while region < total_regions:
        ...
        while s_w_h_d < 3:
            ...
            plt.savefig('../Results' + '48 Time slices ' + name[region] + ' seasonal ' + s_w_h_ds[s_w_h_d])
    ...
    while region < total_regions:
        ...
        plt.savefig('../Results' + '48 Time slices ' + name[region] + ' annual ' + s_w_h_ds[s_w_h_d])
