...
path = "/Users/marek/Documents/Github/zz22yssp/time_slice1/py/"
...
# Cycle to create the time slices for all regions, working one by one as shown above.
while region < total_regions:
    ...
    # Calculating separately Winter, Spring, Summer and Autumn.

    # For each season,  there are 547.5 comparisons required.
    # For each comparison, the 2190 hours will be compared with the 2 days of the representative week.

    # (minimum sum non 0,w,d1,h1,h2,h3,h4,)  w: numbers of week (547/42),一周42个片段，平均每季13周, d1: most representative day,

    w_minimum = [10000000000, 0, 0, 0, 0, 0, 0]     # 四个季节分别算，[比大小, w, d1, h1, h2, h3, h4]
    sp_minimum = [10000000000, 0, 0, 0, 0, 0, 0]
    su_minimum = [10000000000, 0, 0, 0, 0, 0, 0]
    a_minimum = [10000000000, 0, 0, 0, 0, 0, 0]

    solar_results_seasonal = np.zeros(shape=48)     # 1行48列空矩阵
    wind_results_seasonal = np.zeros(shape=48)
    hydro_results_seasonal = np.zeros(shape=48)
    demand_results_seasonal = np.zeros(shape=48)

    solar_results_sorted_seasonal = np.zeros(shape=48)      # sorted results
    ...
    while w < 13:   # 13 = 547//(6*7)
        ...
        x2 = 0
        s_w_4_sorted1 = solar_4h[w * 42:w * 42 + 42]    # 按周筛选，42个片段，solar_4h[0:42], solar_4h[42:84], [504,546]
                ...
                while h2 < 34:
                    ...
                    while h3 < 33:
                        ...
                        while h4 < 32:
                            ...
                            s_w_4_sorted = np.sort(np.concatenate([s_w_4_sorted1[d1:d1 + 6],
                                                                   [s_w_4_sorted1[h1], s_w_4_sorted1[h2],
                                                                    s_w_4_sorted1[h3], s_w_4_sorted1[h4],
                                                                    solar_load_curve_Wi.max(),
                                                                    solar_load_curve_Wi.min()]]))[::-1]     # 每天6个片段，从高到低排序，成1列
                            s_sp_4_sorted = np.sort(np.concatenate([s_sp_4_sorted1[d1:d1 + 6],
                                                                    [s_sp_4_sorted1[h1], s_sp_4_sorted1[h2],
                                                                     s_sp_4_sorted1[h3], s_sp_4_sorted1[h4],
                                                                     solar_load_curve_Sp.max(),
                                                                     solar_load_curve_Sp.min()]]))[::-1]
                            s_su_4_sorted = np.sort(np.concatenate([s_su_4_sorted1[d1:d1 + 6],
                                                                    [s_su_4_sorted1[h1], s_su_4_sorted1[h2],
                                                                     s_su_4_sorted1[h3], s_su_4_sorted1[h4],
                                                                     solar_load_curve_Su.max(),
                                                                     solar_load_curve_Su.min()]]))[::-1]
                            s_a_4_sorted = np.sort(np.concatenate([s_a_4_sorted1[d1:d1 + 6],
                                                                   [s_a_4_sorted1[h1], s_a_4_sorted1[h2],
                                                                    s_a_4_sorted1[h3], s_a_4_sorted1[h4],
                                                                    solar_load_curve_Au.max(),
                                                                    solar_load_curve_Au.min()]]))[::-1]

                            w_w_4_sorted = np.sort(np.concatenate([w_w_4_sorted1[d1:d1 + 6],
                                                                   [w_w_4_sorted1[h1], w_w_4_sorted1[h2],
                                                                    w_w_4_sorted1[h3], w_w_4_sorted1[h4],
                                                                    wind_load_curve_Wi.max(),
                                                                    wind_load_curve_Wi.min()]]))[::-1]
                            w_sp_4_sorted = np.sort(np.concatenate([w_sp_4_sorted1[d1:d1 + 6],
                                                                    [w_sp_4_sorted1[h1], w_sp_4_sorted1[h2],
                                                                     w_sp_4_sorted1[h3], w_sp_4_sorted1[h4],
                                                                     wind_load_curve_Sp.max(),
                                                                     wind_load_curve_Sp.min()]]))[::-1]
                            w_su_4_sorted = np.sort(np.concatenate([w_su_4_sorted1[d1:d1 + 6],
                                                                    [w_su_4_sorted1[h1], w_su_4_sorted1[h2],
                                                                     w_su_4_sorted1[h3], w_su_4_sorted1[h4],
                                                                     wind_load_curve_Su.max(),
                                                                     wind_load_curve_Su.min()]]))[::-1]
                            w_a_4_sorted = np.sort(np.concatenate([w_a_4_sorted1[d1:d1 + 6],
                                                                   [w_a_4_sorted1[h1], w_a_4_sorted1[h2],
                                                                    w_a_4_sorted1[h3], w_a_4_sorted1[h4],
                                                                    wind_load_curve_Au.max(),
                                                                    wind_load_curve_Au.min()]]))[::-1]

                            h_w_4_sorted = np.sort(np.concatenate([h_w_4_sorted1[d1:d1 + 6],
                                                                   [h_w_4_sorted1[h1], h_w_4_sorted1[h2],
                                                                    h_w_4_sorted1[h3], h_w_4_sorted1[h4],
                                                                    hydro_load_curve_Wi.max(),
                                                                    hydro_load_curve_Wi.min()]]))[::-1]
                            h_sp_4_sorted = np.sort(np.concatenate([h_sp_4_sorted1[d1:d1 + 6],
                                                                    [h_sp_4_sorted1[h1], h_sp_4_sorted1[h2],
                                                                     h_sp_4_sorted1[h3], h_sp_4_sorted1[h4],
                                                                     hydro_load_curve_Sp.max(),
                                                                     hydro_load_curve_Sp.min()]]))[::-1]
                            h_su_4_sorted = np.sort(np.concatenate([h_su_4_sorted1[d1:d1 + 6],
                                                                    [h_su_4_sorted1[h1], h_su_4_sorted1[h2],
                                                                     h_su_4_sorted1[h3], h_su_4_sorted1[h4],
                                                                     hydro_load_curve_Su.max(),
                                                                     hydro_load_curve_Su.min()]]))[::-1]
                            h_a_4_sorted = np.sort(np.concatenate([h_a_4_sorted1[d1:d1 + 6],
                                                                   [h_a_4_sorted1[h1], h_a_4_sorted1[h2],
                                                                    h_a_4_sorted1[h3], h_a_4_sorted1[h4],
                                                                    hydro_load_curve_Au.max(),
                                                                    hydro_load_curve_Au.min()]]))[::-1]

                            d_w_4_sorted = np.sort(np.concatenate([d_w_4_sorted1[d1:d1 + 6],
                                                                   [d_w_4_sorted1[h1], d_w_4_sorted1[h2],
                                                                    d_w_4_sorted1[h3], d_w_4_sorted1[h4],
                                                                    demand_load_curve_Wi.max(),
                                                                    demand_load_curve_Wi.min()]]))[::-1]
                            d_sp_4_sorted = np.sort(np.concatenate([d_sp_4_sorted1[d1:d1 + 6],
                                                                    [d_sp_4_sorted1[h1], d_sp_4_sorted1[h2],
                                                                     d_sp_4_sorted1[h3], d_sp_4_sorted1[h4],
                                                                     demand_load_curve_Sp.max(),
                                                                     demand_load_curve_Sp.min()]]))[::-1]
                            d_su_4_sorted = np.sort(np.concatenate([d_su_4_sorted1[d1:d1 + 6],
                                                                    [d_su_4_sorted1[h1], d_su_4_sorted1[h2],
                                                                     d_su_4_sorted1[h3], d_su_4_sorted1[h4],
                                                                     demand_load_curve_Su.max(),
                                                                     demand_load_curve_Su.min()]]))[::-1]
                            d_a_4_sorted = np.sort(np.concatenate([d_a_4_sorted1[d1:d1 + 6],
                                                                   [d_a_4_sorted1[h1], d_a_4_sorted1[h2],
                                                                    d_a_4_sorted1[h3], d_a_4_sorted1[h4],
                                                                    demand_load_curve_Au.max(),
                                                                    demand_load_curve_Au.min()]]))[::-1]

                            x2 = 182    # 182 = 8760 // 48
                            sum1 = 0    # distance in each season
                            sum2 = 0
                            sum3 = 0
                            sum4 = 0
                            while x2 < 2190 - 182:
                                ...
                                sum1 = sum1 + ((hydro_load_curve_Wi[x2] - h_w_4_sorted[
                                    int(x2 / 182.5)]) ** 2) / 3  # Hydropower is not well represented in the model, so we reduce the impact of hydropower on the final results.
                            ...
                            if sum1 != 0 and sum1 < w_minimum[0]:
                                ...
                                solar_results_seasonal[0:12] = np.concatenate([s_w_4_sorted1[d1:d1 + 6],
                                                                               [s_w_4_sorted1[h1],
                                                                                s_w_4_sorted1[h2],
                                                                                s_w_4_sorted1[h3],
                                                                                s_w_4_sorted1[h4],
                                                                                solar_load_curve_Wi.max(),
                                                                                solar_load_curve_Wi.min()]])
                                wind_results_seasonal[0:12] = np.concatenate([w_w_4_sorted1[d1:d1 + 6],
                                                                              [w_w_4_sorted1[h1], w_w_4_sorted1[h2],
                                                                               w_w_4_sorted1[h3], w_w_4_sorted1[h4],
                                                                               wind_load_curve_Wi.max(),
                                                                               wind_load_curve_Wi.min()]])
                                hydro_results_seasonal[0:12] = np.concatenate([h_w_4_sorted1[d1:d1 + 6],
                                                                               [h_w_4_sorted1[h1],
                                                                                h_w_4_sorted1[h2],
                                                                                h_w_4_sorted1[h3],
                                                                                h_w_4_sorted1[h4],
                                                                                hydro_load_curve_Wi.max(),
                                                                                hydro_load_curve_Wi.min()]])
                                demand_results_seasonal[0:12] = np.concatenate([d_w_4_sorted1[d1:d1 + 6],
                                                                                [d_w_4_sorted1[h1],
                                                                                 d_w_4_sorted1[h2],
                                                                                 d_w_4_sorted1[h3],
                                                                                 d_w_4_sorted1[h4],
                                                                                 demand_load_curve_Wi.max(),
                                                                                 demand_load_curve_Wi.min()]])
                                solar_results_sorted_seasonal[0:12] = s_w_4_sorted  # 从高到低排序
                            ...
                            if sum2 != 0 and sum2 < sp_minimum[0]:
                                ...
                                solar_results_seasonal[12:24] = np.concatenate([s_sp_4_sorted1[d1:d1 + 6],
                                                                                [s_sp_4_sorted1[h1],
                                                                                 s_sp_4_sorted1[h2],
                                                                                 s_sp_4_sorted1[h3],
                                                                                 s_sp_4_sorted1[h4],
                                                                                 solar_load_curve_Sp.max(),
                                                                                 solar_load_curve_Sp.min()]])
                                wind_results_seasonal[12:24] = np.concatenate([w_sp_4_sorted1[d1:d1 + 6],
                                                                               [w_sp_4_sorted1[h1],
                                                                                w_sp_4_sorted1[h2],
                                                                                w_sp_4_sorted1[h3],
                                                                                w_sp_4_sorted1[h4],
                                                                                wind_load_curve_Sp.max(),
                                                                                wind_load_curve_Sp.min()]])
                                hydro_results_seasonal[12:24] = np.concatenate([h_sp_4_sorted1[d1:d1 + 6],
                                                                                [h_sp_4_sorted1[h1],
                                                                                 h_sp_4_sorted1[h2],
                                                                                 h_sp_4_sorted1[h3],
                                                                                 h_sp_4_sorted1[h4],
                                                                                 hydro_load_curve_Sp.max(),
                                                                                 hydro_load_curve_Sp.min()]])
                                demand_results_seasonal[12:24] = np.concatenate([d_sp_4_sorted1[d1:d1 + 6],
                                                                                 [d_sp_4_sorted1[h1],
                                                                                  d_sp_4_sorted1[h2],
                                                                                  d_sp_4_sorted1[h3],
                                                                                  d_sp_4_sorted1[h4],
                                                                                  demand_load_curve_Sp.max(),
                                                                                  demand_load_curve_Sp.min()]])
                            ...
                            if sum3 != 0 and sum3 < su_minimum[0]:
                                ...
                                solar_results_seasonal[24:36] = np.concatenate([s_su_4_sorted1[d1:d1 + 6],
                                                                                [s_su_4_sorted1[h1],
                                                                                 s_su_4_sorted1[h2],
                                                                                 s_su_4_sorted1[h3],
                                                                                 s_su_4_sorted1[h4],
                                                                                 solar_load_curve_Su.max(),
                                                                                 solar_load_curve_Su.min()]])
                                wind_results_seasonal[24:36] = np.concatenate([w_su_4_sorted1[d1:d1 + 6],
                                                                               [w_su_4_sorted1[h1],
                                                                                w_su_4_sorted1[h2],
                                                                                w_su_4_sorted1[h3],
                                                                                w_su_4_sorted1[h4],
                                                                                wind_load_curve_Su.max(),
                                                                                wind_load_curve_Su.min()]])
                                hydro_results_seasonal[24:36] = np.concatenate([h_su_4_sorted1[d1:d1 + 6],
                                                                                [h_su_4_sorted1[h1],
                                                                                 h_su_4_sorted1[h2],
                                                                                 h_su_4_sorted1[h3],
                                                                                 h_su_4_sorted1[h4],
                                                                                 hydro_load_curve_Su.max(),
                                                                                 hydro_load_curve_Su.min()]])
                                demand_results_seasonal[24:36] = np.concatenate([d_su_4_sorted1[d1:d1 + 6],
                                                                                 [d_su_4_sorted1[h1],
                                                                                  d_su_4_sorted1[h2],
                                                                                  d_su_4_sorted1[h3],
                                                                                  d_su_4_sorted1[h4],
                                                                                  demand_load_curve_Su.max(),
                                                                                  demand_load_curve_Su.min()]])
                            ...
                            if sum4 != 0 and sum4 < a_minimum[0]:
                                ...
                                solar_results_seasonal[36:48] = np.concatenate([s_a_4_sorted1[d1:d1 + 6],
                                                                                [s_a_4_sorted1[h1],
                                                                                 s_a_4_sorted1[h2],
                                                                                 s_a_4_sorted1[h3],
                                                                                 s_a_4_sorted1[h4],
                                                                                 solar_load_curve_Au.max(),
                                                                                 solar_load_curve_Au.min()]])
                                wind_results_seasonal[36:48] = np.concatenate([w_a_4_sorted1[d1:d1 + 6],
                                                                               [w_a_4_sorted1[h1],
                                                                                w_a_4_sorted1[h2],
                                                                                w_a_4_sorted1[h3],
                                                                                w_a_4_sorted1[h4],
                                                                                wind_load_curve_Au.max(),
                                                                                wind_load_curve_Au.min()]])
                                hydro_results_seasonal[36:48] = np.concatenate([h_a_4_sorted1[d1:d1 + 6],
                                                                                [h_a_4_sorted1[h1],
                                                                                 h_a_4_sorted1[h2],
                                                                                 h_a_4_sorted1[h3],
                                                                                 h_a_4_sorted1[h4],
                                                                                 hydro_load_curve_Au.max(),
                                                                                 hydro_load_curve_Au.min()]])
                                demand_results_seasonal[36:48] = np.concatenate([d_a_4_sorted1[d1:d1 + 6],
                                                                                 [d_a_4_sorted1[h1],
                                                                                  d_a_4_sorted1[h2],
                                                                                  d_a_4_sorted1[h3],
                                                                                  d_a_4_sorted1[h4],
                                                                                  demand_load_curve_Au.max(),
                                                                                  demand_load_curve_Au.min()]])
    ...
    solar_and_wind_seasonal = np.vstack((solar_and_wind_seasonal, solar_results_seasonal, wind_results_seasonal,
                                         hydro_results_seasonal, demand_results_seasonal))
    solar_and_wind_sorter_seasonal = np.vstack((solar_and_wind_sorter_seasonal, solar_results_sorted_seasonal,
                                                wind_results_sorted_seasonal, hydro_results_sorted_seasonal,
                                                demand_results_sorted_seasonal))
    solar_and_wind_duration_curve_seasonal = np.vstack((solar_and_wind_duration_curve_seasonal,
                                                        solar_load_curve_seasonal, wind_load_curve_seasonal,
                                                        hydro_load_curve_seasonal, demand_load_curve_seasonal))

    solar_and_wind_annual = np.vstack((solar_and_wind_annual, solar_results_annual, wind_results_annual,
                                       hydro_results_annual, demand_results_annual))
    solar_and_wind_sorter_annual = np.vstack((solar_and_wind_sorter_annual, solar_results_sorted_annual,
                                              wind_results_sorted_annual, hydro_results_sorted_annual,
                                              demand_results_sorted_annual))
    solar_and_wind_duration_curve_annual = np.vstack((solar_and_wind_duration_curve_annual, solar_load_curve_annual,
                                                      wind_load_curve_annual, hydro_load_curve_annual,
                                                      demand_load_curve_annual))
...
while region < total_regions:
    ...
    while s_w_h_d < 4:
        ...
        ax.set_xlim([1, 12])
        ax.set_ylim([0, 1])
        ax2.plot(x2_axis[1:2191], solar_and_wind_duration_curve_seasonal[n, 2190 + 2190:2190 + 2190 + 2190],
                 color="r")
        ax2.get_xaxis().set_visible(False)
        ax2.get_yaxis().set_visible(False)
        ax2.set_xlim([1, 2190])
        ...
        ax.set_xlim([1, 12])
        ax.set_ylim([0, 1])
        ax2.plot(x2_axis[1:2191],
                 solar_and_wind_duration_curve_seasonal[n, 2190 + 2190 + 2190:2190 + 2190 + 2190 + 2190], color="r")
        ax2.get_xaxis().set_visible(False)
        ax2.get_yaxis().set_visible(False)
        ax2.set_xlim([1, 2190])
        ax2.set_ylim([0, 1])
